...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($is_new_patient) {
            ...
            $stmt = $conn->prepare("
                INSERT INTO patients (patient_id, name, phone, email, dob, age, gender, blood_group, address, emergency_contact_name, emergency_contact_phone)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->bind_param("sssssssssss", 
                $new_patient_id, $patient_name, $patient_phone, $patient_email,
                $patient_dob ?: null, $patient_age, $patient_gender, 
                $patient_blood ?: null, $patient_address, $emergency_name, $emergency_phone
            );
            $stmt->execute();
        ...
        $stmt->bind_param("siiisi", $visit_id, $db_patient_id, $department_id, $doc_val, $token);
        $stmt->execute();
...
<div class="relative min-h-screen group-data-[sidebar-size=sm]:min-h-sm">
                    ...
                    <div class="card-body">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                            ...
                            <div class="md:col-span-3 flex items-end">
                                <button type="button" 
                                        @click="searchPatient()" 
                                        :disabled="searchPhone.length < 11 || searching"
                            ...
                            <div class="md:col-span-4 flex items-end">
                                ...
                                <div x-show="searchDone && patientFound" class="flex items-center gap-2 p-2 rounded-md bg-sky-50 dark:bg-sky-500/10 border border-sky-200 dark:border-sky-500/20 w-full">
                                    ...
                                    <span class="text-xs text-slate-500 dark:text-zink-200">(<?= '{visit_count}' ?> visits)</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                            ...
                            <div class="md:col-span-4">
                                ...
                                <div class="flex items-center gap-2 px-3 py-[9px] rounded-md bg-slate-100 dark:bg-zink-600 border border-slate-200 dark:border-zink-500">
    ...
    function registerForm() {
        return {
            ...
            async searchPatient() {
                if (this.searchPhone.length < 11) return;
