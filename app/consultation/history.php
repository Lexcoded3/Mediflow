...
$dept_stmt = $pdo->prepare("SELECT id, name FROM departments WHERE id = ? LIMIT 1");
...
// ── 3. Build main query ──────────────────────────────────────────────────────
$where_parts = ["v.department_id = :dept_id"];
$params      = [':dept_id' => $dept_id];

if ($filter_status) {
    $where_parts[] = "v.status = :status";
    $params[':status'] = $filter_status;
}
if ($filter_doctor) {
    $where_parts[] = "v.doctor_id = :doctor_id";
    $params[':doctor_id'] = $filter_doctor;
}
if ($filter_date_from) {
    $where_parts[] = "v.visit_date >= :date_from";
    $params[':date_from'] = $filter_date_from;
}
if ($filter_date_to) {
    $where_parts[] = "v.visit_date <= :date_to";
    $params[':date_to'] = $filter_date_to;
}
if ($search_query) {
    $where_parts[] = "(p.name LIKE :q OR p.patient_id LIKE :q2 OR c.diagnosis LIKE :q3 OR c.chief_complaint LIKE :q4)";
...
");
$visits_stmt->execute($params);
$all_visits = $visits_stmt->fetchAll(PDO::FETCH_ASSOC);
...
$doc_stmt = $pdo->prepare("
    ...
    ORDER  BY d.name
");
$doc_stmt->execute([$dept_id]);
$dept_doctors = $doc_stmt->fetchAll(PDO::FETCH_ASSOC);
