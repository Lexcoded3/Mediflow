...
if ($q && strlen($q) >= 2) {
    ...
    $search_results['patients'] = $conn->query("
        SELECT patient_id, name, phone FROM patients
        WHERE name LIKE '$like' OR patient_id LIKE '$like' OR phone LIKE '$like'
        LIMIT 5
    ");
    $search_results['visits'] = $conn->query("
        SELECT v.visit_id, v.status, v.token_number, p.name AS patient_name
        FROM visits v
        JOIN patients p ON p.id = v.patient_id
        WHERE v.visit_id LIKE '$like' OR p.name LIKE '$like'
...
// Admin / Reception: new patients registered today
...
// Triage / Admin: patients waiting for triage
...
// Doctor / Admin: patients in triage (ready for consultation)
if (in_array($role, ['admin', 'doctor'])) {
    $res = $conn->query("
        ...
        ORDER BY v.updated_at ASC LIMIT 10
    ");
    while ($r = $res->fetch_assoc()) {
        $notifications[] = [
            'icon'  => 'stethoscope',
...
if (in_array($role, ['admin', 'lab_technician'])) {
    $res = $conn->query("
        ...
        JOIN patients p ON p.id = v.patient_id
        WHERE lo.status = 'ordered' AND v.visit_date = CURDATE()
        ORDER BY lo.ordered_at ASC LIMIT 10
    ");
    while ($r = $res->fetch_assoc()) {
        $notifications[] = [
            'icon'  => 'test-tube',
...
// Pharmacy: prescriptions pending dispense
...
// Billing / Admin: completed visits not yet billed
                    ...
                    <?php endif; ?>
                        ...
                        <div class="relative flex items-center dropdown h-header">
                            ...
                            <div class="absolute z-50 hidden p-4 ltr:text-left rtl:text-right bg-white rounded-md shadow-md !top-4 dropdown-menu min-w-[14rem] dark:bg-zink-600" aria-labelledby="dropdownMenuButton">
                                ...
                                <ul>
                                    <li>
                                        <a class="flex items-center gap-2 px-2 py-1.5 text-sm text-slate-600 dark:text-zink-200 rounded hover:bg-slate-50 dark:hover:bg-zink-500 hover:text-custom-500 dark:hover:text-custom-500 transition-colors" href="#"><i data-lucide="user-2" class="inline-block size-4 ltr:mr-2 rtl:ml-2"></i> Profile</a>
                                    </li>
                                    <?php if (isAdmin()): ?>
                                        <li>
